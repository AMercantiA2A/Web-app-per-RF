...
if st.button("Esegui l'unione dei dataset"):
    try:
        df1 = pd.read_excel(uploaded_files["Gateway attivi (Excel)"])
        df2 = pd.read_csv(uploaded_files["Device Gateway Performance (CSV)"])
        df3 = pd.read_csv(uploaded_files["Meter A2A (CSV)"], sep=';', encoding='latin-1')
        ...
        def haversine(lat1, lon1, lat2, lon2):
            ...
            return np.ceil((R * c) * 1000)

        df_finale_nec['DISTANZA Meter-Gateway [m]'] = haversine(
            df_finale_nec['LATITUDINE METER'], df_finale_nec['LONGITUDINE METER'],
            df_finale_nec['LATITUDINE GATEWAY'], df_finale_nec['LONGITUDINE GATEWAY']
        )
